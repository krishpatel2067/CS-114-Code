  ...
  public String levelOrder()
  {
	  ...
	  if (root.left() != null || root.right() != null)
	  {
		  for (int lvl = 1; lvl < height; lvl++)
		  {
			  ...
			  BSTNode<K, E>[] currLvlNodes = new BSTNode[(int) Math.pow(2, lvl)];	// max num of nodes in this level 2^lvl
...
//	  return levelOrderHelp(root, "" + root.element());
	  return str;
  }
  ...
  private BSTNode<K, E> inserthelp(BSTNode<K, E> rt, K k, E e, int depthSoFar) {
    if (rt == null)
    {
    	if (depthSoFar > this.height)
    		this.height++;
    	return new BSTNode<K, E>(k, e);
    }
    if (rt.key().compareTo(k) > 0)
      rt.setLeft(inserthelp(rt.left(), k, e, ++depthSoFar));
    else
      rt.setRight(inserthelp(rt.right(), k, e, ++depthSoFar));
    return rt;
  }
