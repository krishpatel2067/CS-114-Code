...
public class Sorts {
    ...
    <E extends Comparable<? super E>> int findpivot(E[] A, int i, int j) {
        return i + rnd.nextInt(j-i+1); // Randomized quicksort.
        //return j;  // Deterministic quickort.
    }
    ...
    <E extends Comparable<E>> void mergesort(E[] A, E[] B, int l, int r) {
        int mid = (l + r) / 2; // Select midpoint
        if (l == r)
            return; // List has one element
        mergesort(A, B, l, mid); // Mergesort first half
        mergesort(A, B, mid + 1, r); // Mergesort second half
        for (int i = l; i <= r; i++)
            // Copy subarray to B
            B[i] = A[i];
        ...
        for (int curr = l; curr <= r; curr++) {
            if (i1 == mid + 1) // Left sublist exhausted
                ...
                A[curr] = B[i1++];
            else if (B[i1].compareTo(B[i2]) <= 0) // Get smaller
                A[curr] = B[i1++];
            else
                A[curr] = B[i2++];
        }
